...
public class interact_with_user {    
    ...
    public static void insert_user(String email, String password) {
        ...
        try {
            PreparedStatement statement = connection.connect.prepareStatement("INSERT INTO [user] (id, email, pass, point) VALUES (?, ?, ?, ?)");
            statement.setInt(1, create_id_user());
            statement.setString(2, email);
            statement.setString(3, password);
    ...
    // insert point
    public static void update_point(float point, int user_id) {
    	connect_database connection = new connect_database();
  	  connection.connect_to_database();

  	  try {
  	    PreparedStatement statement = connection.connect.prepareStatement("UPDATE [user] set point = point + " + point + " WHERE id = ?");
    ...
    public static float get_point(int user_id) {
    	connect_database connection = new connect_database();
        connection.connect_to_database();
        float result = 0;
        try {
            Statement statement = connection.connect.createStatement();
            String sql = "select point from [user] where id = " + user_id;
            ResultSet resultSet = statement.executeQuery(sql);
