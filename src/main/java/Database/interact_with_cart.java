...
//        DecimalFormat df = new DecimalFormat("#.###");
    ...
    public static void delete_product_from_cart(int user_id, int product_id, float size) {
        try {
            String query = "DELETE FROM cart "
                    + "  where id_user = ? and id_product = ? and size = ?";
            float a = (float)size;
            PreparedStatement statement = conn.prepareStatement(query);
            statement.setInt(1, user_id); // assuming you want to append the user ID with an underscore separator
    ...
    public static void main(String args[]) {
    	float size = (float)40.0;
    	System.out.println(size);
    	interact_with_cart.delete_product_from_cart(1, 25, size);
    }
    ...
    //UPDATE CART KHI BUY NOW UPDATE (TON TAI SAN PHAM TRONG CART)

    public static void update_cart_quantity(int id_user, int id_product, float size, int quantity) {
        try {
            String query = "UPDATE cart SET quantity = ? WHERE id_user = ? AND id_product = ? AND size = ?";
            PreparedStatement preparedStatement = conn.prepareStatement(query);
            preparedStatement.setInt(2, id_user);
            preparedStatement.setInt(3, id_product);
            preparedStatement.setFloat(4, size);
            preparedStatement.setInt(1, quantity);

            int rowsAffected = preparedStatement.executeUpdate();
            if (rowsAffected > 0) {
                System.out.println("A new cart was updated successfully!");
            } else {
                System.out.println("A new cart was not updated successfully!");
            }
    ...
    public static int get_product_in_cart(int user_id) {
    	int result = 0;
        try {
